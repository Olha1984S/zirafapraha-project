...
export class ZPCheckingFooterMenuLinksMobileP4 {
  ...
  /** Проверка ссылок на вспомогательные страницы (четвёртый блок, мобильная версия) */
  async checkFooterMenuItemsMobileP4() {
    console.log('\n🟦 Проверка ссылок в четвёртом блоке footer (мобильная версия)...');
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (href.includes('zirafapraha.cz/')) {
        ...
        // Формируем безопасное имя файла и временную метку
    ...
    console.log('\n✅ Проверка четвёртого блока footer для мобильной версии завершена!');
  }
}
