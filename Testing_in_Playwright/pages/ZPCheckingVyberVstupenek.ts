...
export class ZPCheckingVyberVstupenek {
  constructor(private page: Page) {}

  /** Проверка работы блока "Výběr vstupenek" */
  async checkingVyberVstupenek() {
    const { page } = this;
...
//     // === ОБЩАЯ СУММА ВСЕХ ВЫБРАННЫХ КАРТОЧЕК ===
    ...
    const subtotal = parseFloat(await page.locator('.js-subtotal-price').textContent() || '0');
    ...
    const totalSummaryElement = page.locator('.entries-summary__total .entry-price-currency');
    ...
    const difference = Math.abs(totalSummary - total);

    console.log(`Проверяем общую цену: на сайте ${totalSummary} Kč, рассчитано ${total} Kč`);
