...
import { ZPVstupenkyPage } from '../pages/ZPVstupenkyPage';
...
test('Проверка выбора колическо и стоимости билетов', async ({ page }) => {
  const vstupenekPage = new ZPVstupenkyPage(page);
  const vyberVstupenek = new ZPCheckingVyberVstupenek(page);

  
  // Открываем главную страницу
  await vstupenekPage.goto();
  await vstupenekPage.acceptCookies();
  await vstupenekPage.lincVstupenky();
